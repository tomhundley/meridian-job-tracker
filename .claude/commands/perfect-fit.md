...
```bash
curl -s "http://localhost:8000/api/jobs?is_perfect_fit=true" | jq '.items[] | {title, company, priority}'
```
...
```bash
curl -s "http://localhost:8000/api/jobs?is_perfect_fit=true&is_ai_forward=true" | jq '.items'
```
