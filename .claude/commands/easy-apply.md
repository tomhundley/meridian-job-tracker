...
```bash
curl -s "http://localhost:8000/api/jobs?is_easy_apply=true" | jq '.items[] | {title, company, url, status}'
```
...
```bash
curl -s "http://localhost:8000/api/jobs?is_easy_apply=true&target_role=vp" | jq '.items'
```

AI-forward Easy Apply jobs:
```bash
curl -s "http://localhost:8000/api/jobs?is_easy_apply=true&is_ai_forward=true" | jq '.items'
```

Ready to apply Easy Apply jobs:
```bash
curl -s "http://localhost:8000/api/jobs?is_easy_apply=true&status=ready_to_apply" | jq '.items'
```
...
```bash
curl -s "http://localhost:8000/api/discovery/stats" | jq '.easy_apply_jobs'
```
